                                                    ...
                                                    <Draggable key={idx} draggableId={String(idx)} index={idx}>
                                                        {(provided, snapshot) => (
                                                            <tr
                                                                ...
                                                                className={`border-b last:border-b-0 transition-colors ${snapshot.isDragging ? 'bg-blue-50' : 'hover:bg-[#f1f5f9]'}`}
                                                            >
                                                                <td className="p-3 align-middle cursor-grab" {...provided.dragHandleProps}>
                                                                    <GripVertical className="w-4 h-4 text-muted-foreground" />
                                                                </td>
                                                                <td className="p-3 font-medium">{stage.name}</td>
                                                                <td className="p-3 text-center"><span className="inline-block w-6 h-6 rounded-full border" style={{ background: stage.color }} /></td>
                                                                <td className="p-3">{stage.description}</td>
                                                                <td className="p-3 text-right">
                                                                    <button className="inline-flex items-center p-1 hover:bg-gray-100 rounded" title="Editar" onClick={() => openEditStageModal(idx)}>
                                                                        <Pencil className="w-4 h-4 text-gray-500" />
                                                                    </button>
                                                                    <button className="inline-flex items-center p-1 hover:bg-gray-100 rounded ml-2" title="Excluir" onClick={() => setConfirmStageIdx(idx)}>
