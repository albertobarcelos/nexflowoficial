...
export function useEntityFields(entityId?: string) {
  ...
  const updateFieldMutation = useMutation({
    mutationFn: async ({ id, ...updates }: EntityFieldUpdate & { id: string }) => {
      const { data, error } = await supabase
        .from('web_entity_fields')
        .update(updates)
        .eq('id', id)
...
export function useEntityRecords(entityId?: string) {
  ...
  const createRecordMutation = useMutation({
    mutationFn: async (recordData: Omit<EntityRecordInsert, 'client_id' | 'created_by'>) => {
      const user = await getCurrentUserData()
      if (!user?.client_id || !user?.id) throw new Error('Usuário não encontrado')
  ...
  const updateRecordMutation = useMutation({
    mutationFn: async ({ id, ...updates }: EntityRecordUpdate & { id: string }) => {
      const user = await getCurrentUserData()
